export function MatchCurves() {
   ...
   this.addGlyph = (name,...args) => {
      ...
      if (typeof args[0] == 'function') {
         sketchClass = args[0];
	 let sketchInstance = new sketchClass();
   ...
   this.recognize = src => {
      ...
      for (let k = 0 ; k < glyphs.length ; k++)
         if (glyphs[k].curves.length == candidate.length) {
            let score = 0;
            for (let i = 0 ; i < glyphs[k].curves.length ; i++) {
               let curveLen = this.curveLength(glyphs[k].curves[i]);
               for (let n = 0 ; n < 100 ; n++) {
                  let dx = candidate[i][n][0] - glyphs[k].curves[i][n][0];
                  let dy = candidate[i][n][1] - glyphs[k].curves[i][n][1];
      ...
      for (let i = 0 ; i < glyphs[K].curves.length ; i++) {
         let curve = [];
         for (let n = 0 ; n < 100 ; n++)
            curve.push([ xys[0] + xys[2] * glyphs[K].curves[i][n][0],
