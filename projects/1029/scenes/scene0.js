// DATA TO CREATE THE LINEFONT
/*
let linefont = [
{ name: ' ', paths: [
] },
{ name: '!', paths: [
  [[30,0],[30,75]],
  [[25,100],[35,100],[35,90],[25,90],[25,100]],
] },
{ name: '"', paths: [
  [[20,0],[20,30]],
  [[40,0],[40,30]],
] },
{ name: '#', paths: [
  [[25,10],[15,90]],
  [[45,10],[35,90]],
  [[0,40],[60,40]],
  [[0,60],[60,60]],
] },
{ name: '$', paths: [
  [[50,20],[10,20],[10,50],[50,50],[50,80],[10,80]],
  [[25,0],[25,100]],
  [[35,0],[35,100]],
] },
{ name: '%', paths: [
  [[60,10],[0,90]],
  [[ 5, 20],[15, 20],[15,10],[ 5,10],[ 5, 20]],
  [[45, 90],[55, 90],[55,80],[45,80],[45, 90]],
] },
{ name: '&', paths: [
  [[60,100],[0,20],[15,0],[35,0],[50,20],[0,70],
      [0,100],[30,100],[60,70]],
] },
{ name: '\'', paths: [
  [[35,0],[25,30]],
] },
{ name: '(', paths: [
  [[40,0],[20,30],[20,70],[40,100]],
] },
{ name: ')', paths: [
  [[20,0],[40,30],[40,70],[20,100]],
] },
{ name: '*', paths: [
  [[30,25],[30,75]],
  [[ 7,35],[53,65]],
  [[53,35],[ 7,65]],
] },
{ name: '+', paths: [
  [[10,60],[50,60]],
  [[30,40],[30,80]],
] },
{ name: ',', paths: [
  [[30,80],[30,100],[15,120]],
] },
{ name: '-', paths: [
  [[10,60],[50,60]],
] },
{ name: '.', paths: [
  [[25,100],[35,100],[35,90],[25,90],[25,100]],
] },
{ name: '/', paths: [
  [[50,0],[10,100]],
] },
{ name: '0', paths: [
  [[15,100],[0,85],[0,15],[15,0],[45,0],[60,15],
      [60,85],[45,100],[15,100]],
] },
{ name: '1', paths: [
  [[10,20],[30,0],[30,100]],
] },
{ name: '2', paths: [
  [[0,30],[0,15],[15,0],[45,0],[60,15],[60,35],
      [0,100],[60,100]],
] },
{ name: '3', paths: [
  [[0,15],[15,0],[45,0],[60,15],[60,35],[40,50],
      [60,65],[60,85],[45,100],[15,100],[0,85]],
] },
{ name: '4', paths: [
  [[60,60],[0,60],[40,0],[40,100]],
] },
{ name: '5', paths: [
  [[60,0],[5,0],[5,40],[45,40],[60,55],[60,85],
      [45,100],[15,100],[0,85]],
] },
{ name: '6', paths: [
  [[0,55],[15,45],[45,45],[60,60],[60,85],[45,100],
      [15,100],[0,85],[0,15],[15,0],[45,0],[60,15]],
] },
{ name: '7', paths: [
  [[0,0],[60,0],[20,100]],
] },
{ name: '8', paths: [
  [[30,50],[45,50],[60,65],[60,85],[45,100],[15,100],
      [0,85],[0,65],[15,50],[40,50],[55,35],[55,15],
      [40,0],[20,0],[5,15],[5,35],[20,50],[30,50]],
] },
{ name: '9', paths: [
  [[60,45],[45,55],[15,55],[0,40],[0,15],[15,0],
      [45,0],[60,15],[60,85],[45,100],[15,100],[0,85]],
] },
{ name: ':', paths: [
  [[25, 50],[35, 50],[35,40],[25,40],[25, 50]],
  [[25,100],[35,100],[35,90],[25,90],[25,100]],
] },
{ name: ';', paths: [
  [[25, 50],[35, 50],[35,40],[25,40],[25, 50]],
  [[30,80],[30,100],[15,120]],
] },
{ name: '<', paths: [
  [[50,30],[10,50],[50,70]],
] },
{ name: '=', paths: [
  [[10,38],[50,38]],
  [[10,62],[50,62]],
] },
{ name: '>', paths: [
  [[10,30],[50,50],[10,70]],
] },
{ name: '?', paths: [
  [[10,20],[10,0],[50,0],[50,40],[30,40],[30,60]],
  [[25,100],[35,100],[35,90],[25,90],[25,100]],
] },
{ name: '0', paths: [
  [[50,115],[15,115],[0,95],[0,30],[15,15],[45,15],
      [60,30],[60,85],[45,97],[25,97],[17,87],[17,40],
      [25,35],[45,35],[60,55]],
] },
{ name: 'A', paths: [
  [[0,100],[30,0],[60,100]],
  [[10,100*2/3],[50,100*2/3]],
] },
{ name: 'B', paths: [
  [[0,100],[0,50],[40,50],[55,65],[55,85],[40,100],[0,100]],
  [[0,50],[0,0],[35,0],[50,15],[50,35],[35,50]],
] },
{ name: 'C', paths: [
  [[60,100],[0,100],[0,0],[60,0]],
] },
{ name: 'D', paths: [
  [[0,100],[0,0],[40,0],[60,20],[60,80],[40,100],
      [0,100]],
] },
{ name: 'E', paths: [
  [[60,100],[0,100],[0,0],[60,0]],
  [[0,50],[40,50],],
] },
{ name: 'F', paths: [
  [[0,100],[0,0],[60,0]],
  [[0,50],[40,50],],
] },
{ name: 'G', paths: [
  [[30,50],[60,50],[60,100],[0,100],[0,0],[60,0]],
] },
{ name: 'H', paths: [
  [[0,0],[0,100]],
  [[60,0],[60,100]],
  [[0,50],[60,50]],
] },
{ name: 'I', paths: [
  [[30,0],[30,100]],
  [[10,0],[50,0]],
  [[10,100],[50,100]],
] },
{ name: 'J', paths: [
  [[0,60],[0,100],[60,100],[60,0]],
] },
{ name: 'K', paths: [
  [[0,0],[0,100]],
  [[60,0],[0,50],[60,100]],
] },
{ name: 'L', paths: [
  [[0,0],[0,100],[60,100]],
] },
{ name: 'M', paths: [
  [[0,100],[0,0],[30,50],[60,0],[60,100]],
] },
{ name: 'N', paths: [
  [[0,100],[0,0],[60,100],[60,0]],
] },
{ name: 'O', paths: [
  [[0,100],[0,0],[60,0],[60,100],[0,100]],
] },
{ name: 'P', paths: [
  [[0,100],[0,0],[60,0],[60,50],[0,50]],
] },
{ name: 'Q', paths: [
  [[0,100],[0,0],[60,0],[60,70],[30,100],[0,100]],
  [[30,70],[60,100]],
] },
{ name: 'R', paths: [
  [[0,100],[0,0],[35,0],[50,15],[50,35],[35,50]],
  [[0,50],[30,50],[60,100]],
] },
{ name: 'S', paths: [
  [[0,100],[60,100],[60,50],[0,50],[0,0],[60,0]],
] },
{ name: 'T', paths: [
  [[30,0],[30,100]],
  [[0,0],[60,0]],
] },
{ name: 'U', paths: [
  [[0,0],[0,100],[60,100],[60,0]],
] },
{ name: 'V', paths: [
  [[0,0],[30,100],[60,0]],
] },
{ name: 'W', paths: [
  [[0,0],[10,100],[30,50],[50,100],[60,0]],
] },
{ name: 'X', paths: [
  [[0,0],[60,100]],
  [[60,0],[0,100]],
] },
{ name: 'Y', paths: [
  [[0,0],[30,50],[60,0]],
  [[30,50],[30,100]],
] },
{ name: 'Z', paths: [
  [[0,0],[60,0],[0,100],[60,100]],
] },
{ name: '[', paths: [
  [[40,0],[20,0],[20,100],[40,100]],
] },
{ name: '\\', paths: [
  [[10,0],[50,100]],
] },
{ name: ']', paths: [
  [[20,0],[40,0],[40,100],[20,100]],
] },
{ name: '^', paths: [
  [[10,30],[30,0],[50,30]],
] },
{ name: '_', paths: [
  [[0,100],[60,100]],
] },
{ name: '`', paths: [
  [[25,0],[35,25]],
] },
{ name: 'a', paths: [
  [[50,75],[30,100],[10,100],[10,50],[50,50],[50,100]],
] },
{ name: 'b', paths: [
  [[10,0],[10,100],[50,100],[50,50],[10,50]],
] },
{ name: 'c', paths: [
  [[50,50],[10,50],[10,100],[50,100]],
] },
{ name: 'd', paths: [
  [[50,0],[50,100],[10,100],[10,50],[50,50]],
] },
{ name: 'e', paths: [
  [[10,75],[50,75],[50,50],[10,50],[10,100],[50,100]],
] },
{ name: 'f', paths: [
  [[50,0],[30,0],[30,100]],
  [[10,50],[50,50]],
] },
{ name: 'g', paths: [
  [[50,100],[10,100],[10,50],[50,50],[50,125],[10,125]],
] },
{ name: 'h', paths: [
  [[10,0],[10,100]],
  [[10,50],[50,50],[50,100]],
] },
{ name: 'i', paths: [
  [[30,50],[30,100]],
  [[25,20],[35,20],[35,10],[25,10],[25,20]],
] },
{ name: 'j', paths: [
  [[50,50],[50,125],[10,125],[10,100]],
  [[45,20],[55,20],[55,10],[45,10],[45,20]],
] },
{ name: 'k', paths: [
  [[10,0],[10,100]],
  [[50,50],[10,75],[50,100]],
] },
{ name: 'l', paths: [
  [[20,0],[30,0],[30,100],[40,100]],
] },
{ name: 'n', paths: [
  [[0,100],[0,50],[15,50],[30,75],[45,50],[60,50],[60,100]],
] },
{ name: 'n', paths: [
  [[10,100],[10,50]],
  [[10,75],[30,50],[50,50],[50,100]],
] },
{ name: 'o', paths: [
  [[50,50],[10,50],[10,100],[50,100],[50,50]],
] },
{ name: 'p', paths: [
  [[10,100],[50,100],[50,50],[10,50],[10,125]],
] },
{ name: 'q', paths: [
  [[50,100],[10,100],[10,50],[50,50],[50,125],[60,125]],
] },
{ name: 'r', paths: [
  [[10,100],[10,50]],
  [[10,75],[30,50],[50,50]],
] },
{ name: 's', paths: [
  [[50,50],[10,50],[10,75],[50,75],[50,100],
      [10,100]],
] },
{ name: 't', paths: [
  [[30,25],[30,100],[50,100]],
  [[10,50],[50,50]],
] },
{ name: 'u', paths: [
  [[50,50],[50,100]],
  [[50,75],[30,100],[10,100],[10,50]],
] },
{ name: 'v', paths: [
  [[10,50],[30,100],[50,50]],
] },
{ name: 'w', paths: [
  [[0,50],[15,100],[30,50],[45,100],[60,50]],
] },
{ name: 'x', paths: [
  [[10,50],[50,100]],
  [[50,50],[10,100]],
] },
{ name: 'y', paths: [
  [[10,50],[30,100]],
  [[20,125],[50,50]],
] },
{ name: 'z', paths: [
  [[10,50],[50,50],[10,100],[50,100]],
] },
{ name: '{', paths: [
  [[40,0],[20,10],[20,40],[10,50],[20,60],[20,90],[40,100]],
] },
{ name: '|', paths: [
  [[30,0],[30,100]],
] },
{ name: '}', paths: [
  [[20,0],[40,10],[40,40],[50,50],[40,60],[40,90],[20,100]],
] },
{ name: '~', paths: [
  [[0,55],[20,45],[40,55],[60,45]],
] },
];
*/
function Scene(){
this.vertexShader=Shader.defaultVertexShader;
this.fragmentShader=Shader.defaultFragmentShader;
}
