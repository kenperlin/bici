function Diagram() {
   ...
   this.update = ctx => {
	 ...
	 this.fillRect([x-.5,-1],[x,C[p]]);
         ...
         this.line([x,-1],[x,1]);
	 this.text(p==0?"y":p==1?"y":p==2?"y'":"y'", [x-.27,.9]);
	 ctx.font = '20px Helvetica';
	 this.text(p==0?"0":p==1?"1":p==2?"0":"1", [x-(p<2?.22:.23),.85]);
	 ...
	 ctx.font = '30px Helvetica';
      ...
      if (P >= 0) {
         ctx.strokeStyle = colors[P];
         this.curve(50, t => [ 2*t-1, C[P] * T(M[P],t) ]);
      }
      ctx.strokeStyle = 'black';
      ctx.lineWidth = 6;
      this.curve(50, t => [ 2*t-1, C[0]*T(M[0],t)+C[1]*T(M[1],t)+C[2]*T(M[2],t)+C[3]*T(M[3],t) ]);
      ctx.lineWidth = 3;
   }
}
