function Diagram() {
   ...
   this.update = ctx => {
	 ...
	 this.drawRect([.3-.35,0-.35],[.3+.35,0+.35]);
         for (let row = 0 ; row < 4 ; row++) {
            this.text('yyyy'.charAt(row), [.4-.03+.4,(1.58-row)/5]);
	    ctx.font = '20px Helvetica';
            this.text('0101'.charAt(row), [.45-.03+.4,(1.58-row)/5-.05]);
	    ctx.font = '30px Helvetica';
            this.text("  ''".charAt(row), [.45-.03+.4,(1.58-row)/5]);
         }
