function Diagram() {
   ...
   this.update = ctx => {
	 ...
	 break;
      ...
      case 6:
         for (let p = 0 ; p < 4 ; p++) {
	    let x = transition((p-1.5)/2, (1.5-p)/2.5, stateTime);
	    let y = -.7;
	    ...
	    let m = M[p];
	    this.curve(20, t => [x-.2+.4*t, y-.2 + .4*(m[0]*t*t*t+m[1]*t*t+m[2]*t+m[3])/6]);
	 }
         ctx.strokeStyle = 'black';
      case 5:
         this.move(.1,0,0);

         this.text('t    t    t   1', [-.2-.55,0]);
         this.text('3   2', [-.32-.55,.1]);

	 this.dot([-.355,0]);

	 ctx.font = '40px Helvetica';
	 this.text('1',[-.16, .1]);
	 this.line([-.21,.02],[-.11,.02]);
	 this.text('6',[-.16,-.1]);
	 ctx.font = '30px Helvetica';
